...
| K4 | 長文で音声生成が失敗/遅延する | **チャンク分割** (§3.3)。文境界を優先し最大 120 トークン (§2.3) で区切り、チャンク単位に合成 |
...
- チャンク長の設定 UI (定数 120 トークン。実測後に調整)
...
2. 文を順に詰め、120 トークン (通常文字 1 字 = 1、Irodori 絵文字は ZWJ/VS16 込みの 1 クラスタ = 1。chars 数ではない) を超えない範囲で 1 チャンクに結合
3. 単一文が 120 トークン超のときは読点 `、` で分割して詰め直す
4. それでも超える断片は 120 トークンで強制分割。ただし絵文字クラスタ (Irodori 45 種、ZWJ/VS16 込み) の途中では切らない
5. 空白のみのチャンクは捨てる
...
1. 短文 1 つ → 1 チャンク
2. 複数短文が 120 トークン以内に結合される
3. 121 字目にかかる文は次チャンクへ (文境界優先)
4. 1 文 120 トークン超 → 読点分割
5. 読点なし 120 トークン超 → 強制分割、かつ char 境界安全
...
| 判断 | 採用値 | 代替案 |
|---|---|---|
| チャンク最大長 | 120 トークン (定数、§2.3) | 設定化 / エンジン別の長さ |
