...
**状態**: 設計 v1.1。**M13・M14・M15 すべて実装済み・実機検証済み**（M13/M15 = 2026-08-10 実装、M14 = 2026-08-16 実装、実機確認は 2026-08-17 に完了）。§7 の未決事項 7 件はすべて決着済み。**v0.4.0 として出荷**
**作成日**: 2026-08-10
...
- **既存コードの実態を正とする**（先行 2 設計書と同じ運用）。**設計時点の**現行は v0.3.0（schema v8、SpeechCategory 12）。architecture.md は実装時に追随改訂する（→ v2.0 で反映済み。本書出荷時点の実装は v0.4.0 / schema v9）。
...
- **生成時刻による失効（本書の設計値。spec §4.4.4 は発話時に鮮度を判定することだけを定め、しきい値は持たない）**: 時事ネタを含まない文も**生成から 30 日を超えたら使わない**。人格や状況が変わっている可能性があり、無期限に持ち越す理由がないため（時事ネタ入りは §3.4 の 7 日が先に効く）。
...
**M15 として 2026-08-10 に完了**。D2 は着手時の全突合で乖離が 1 件ではなく **8 件**あったため、まとめて解消した。

| # | 項目 | 作業 |
|---|---|---|
| D1 | test-plan §5 に日常支援（spec §4.6）の手動チェックが無い | ✅ **G 節を新設**（G-1〜G-19。リマインダー / ToDo・日課 / 状況発話とガバナンス / カレンダーの 4 群。A〜F が spec §4.1〜§4.7 に 1 対 1 対応する既存構造に合わせた）。横断方針を §5.8、実機検証記録を §5.9 へ繰り下げ、参照元（reader 系 spec・releasing-ugg skill）も追随 |
| D2 | architecture.md のモジュール一覧が実体と乖離 | ✅ **8 件を解消**（想定は 1 件だった）。削除: `monologue.rs`（M7 で廃止済み）/ 改名: `asset_dnd.rs` → `dnd.rs` / **追記: `tasks.rs`（全 watcher の実体。記載漏れ）・`irodori_download.rs`・`sidecar.rs`・`gpu.rs`・`manual.rs`**。突合スクリプトで乖離ゼロを確認 |
| D3 | reader 系 2 spec のヘッダが「状態: レビュー待ち」のまま | ✅ 実態（v0.1.1 / v0.1.2 で出荷・実機検証済み・spec/test-plan へ反映済み）に更新 |
| D4 | Irodori 上流の版固定が非一貫 | ✅ **3 資産すべてを commit 固定に統一**（本体 `8224dafb…`・dacvae `414c2078…`。silentcipher は元から pin 済み）。更新時は手で上げて実機で DL・合成まで通す旨をコメントに明記 |
...
1. **モニタ切替で WebView2 に `resize` が発火するか**（発火しなければ `set_monitor_pref` 後に明示イベントで再計算させる）→ **発火する**（2026-08-17、3 モニタ実機 / test-plan A-13）。切替後もクリック透過・キャラ X 位置の clamp・吹き出し追従がいずれも崩れず、明示イベントの追加は不要だった
2. `list_recent_topics` に鮮度フィルタを足すか、呼び出し側で除外するか → **呼び出し側で除外**（M14 決着）。`list_recent_topics` は `fetched_ts DESC` なので、上位 N 件に鮮度フィルタを掛ければ十分（除外される行は残る行より必ず古い）。既存シグネチャを変えない
3. 補充の閾値・バッチ件数・最短間隔（3 / 5 / 30 分は初期値）→ **初期値のまま採用**（M14）。実測での調整は実機確認後に持ち越す
4. 独り言生成のプロンプト文面（ゴースト設定の渡し方・時事ネタの見出しの見せ方・JSON 形式）→ **確定**（M14、`system/monologue.rs::build_system_prompt`）。ゴースト名 + main キャラ名 + pose 語彙（`advanced::available_pose_names` を再利用）／見出しは箇条書きで渡し「要約や事実の断定はせず感想程度に」と制約／出力は `[{"text","pose"}]` の JSON 配列。**会話履歴・`user_profile` は渡さない**（§1.1）
5. モニタ select の表示文字列（順番・解像度・主/現在の示し方）→ M13
6. ストック上限（際限なく溜めない上限値。補充バッチと合わせて決める）→ **20**（M14）。閾値 3 + バッチ 5 なら定常は 8 件前後で、20 は積み過ぎの安全網
7. `apply_dock` の順序変更（`set_position` → `set_size`）が DPI 混在環境で狙いどおり効くか → **効く**（2026-08-17、3 モニタ実機 / test-plan A-13）。拡大率の異なるモニタへ移してもステージ高さが移動先のスケールで再計算され、足元がタスクバー上端に揃う

### 7.1 M14 で設計から意図的に変えた点

| 箇所 | 設計 §3 の記述 | 実装 | 理由 |
|---|---|---|---|
| `count_monologue_cache` | `count_monologue_cache(ghost_id)` | `count_monologue_cache(ghost_id, now)` | pop と**同じ条件**で数えないと、失効済みの行を在庫と誤認して補充が止まる（pop は全部捨てて辞書に落ちているのに「在庫あり」に見える）。失効判定に `now` が要る |
| API キー無しの扱い | 「API キー無し → 何もしない」 | **None も通す**（呼び出しは行い、失敗したら辞書へ） | base_url を差し替えたローカル LLM（LMStudio/Ollama）はキー無しで動く。ここだけ None を拒むと、advanced チャットが動く環境で独り言だけ補充されない。対話チャット（`try_advanced`）も None を通しており、そちらに揃えた |
| `MonologueCacheRow` の列 | 明記なし | `text` / `pose` / `topic_fetched_ts` のみ | `ghost_id` と `created_ts` は pop 時の選別で使い切るため、消費側に用途が無い（規律 2「将来のために入れない」） |
| 消費時の降格判定 | §3.3 の消費フローは `mode == advanced` のみを見る（降格は補充側の話として書かれている） | **降格中は pop せず辞書へ落とす** | spec §4.4.4 が「降格中・LLM 障害時は low と同じ辞書経路にフォールバックする」と明示している。ストックを喋るだけなら課金は無いが、**要件は「降格中は advanced の顔を出さない」と読める**ので spec に従った。ストックは消費されず、復帰後にそのまま使える |
| 補充のゲート | §3.3 は mode / 降格 / 在庫 / 最短間隔 / 上限 | **`monologue_interval_min == 0`（独り言 OFF）も追加** | 独り言を切っているユーザーの裏で、**絶対に喋られない文のために** 30 分ごとに課金 API が呼ばれ続ける。spec §4.4.4「0 設定で無効化可能」の素直な帰結 |
| ストックの鍵 | §3.2 は「現在の `settings.ghost_id`」 | **読み込み済み `bundle.ghost.id`** | ゴースト切替はホットリロードされず再起動が要る（spec §4.5.2）ため、切替直後〜再起動前は `settings.ghost_id`（新）と実際に喋っている人格（旧）がズレる。`settings` 側を鍵にすると「旧ゴーストの人格で生成した文を新ゴーストの行として積む」ことになり、**`ghost_id` を持たせた意味そのものが消える** |
| 織り込みの鮮度条件 | §3.4 は「7 日以内の見出し」 | **7 日以内 かつ 残り寿命 24 時間以上** | 期限ぎりぎり（残り数分）の見出しを織り込むと、積んだ文が**発話時失効でほぼ即座に捨てられ**、在庫 0 のまま 30 分ごとに補充を繰り返す「積んでは捨てる」有料ループになる。最短間隔だけでは止まらない |
| 材料のトピック絞り込み | 記述なし | **`interest_topics` で有効なトピックの見出しだけ** | 興味分野から外したキーワードの見出しは `topics_cache` に最大 7 日残る。絞らないと、外したはずの話題を 1 週間喋り続ける（時事ネタ同意の撤回時に掃除するのと同じ考え方） |
| 補充のタイムアウト | §3.5「専用タイムアウト（`polish_script` の 20 秒に倣う）」 | **120 秒** | **実機で 20 秒では足りなかった**（2026-08-17）。`polish_script` は一文の言い換えで出力数十トークンだが、こちらは**独り言 5 件をまとめて生成**するので出力量が桁違い。LM Studio 実測でウォーム状態でも 22〜44 秒かかり、**ローカル LLM では補充が絶対に成功しない**状態だった。spec §4.2.2 は LMStudio/Ollama を正式サポートしており、`LlmClient` 自身も「ローカル LLM は 60 秒を超えることがある」として全体 180 秒を取っている。待つのは tick の**発話判定より後**なので、伸びても遅れるのは次 tick の判定だけ（最短間隔 30 分に 1 回・最大 2 分）で、既定 10 分間隔の独り言には実質影響しない |
...
| 2026-09-17 | v1.1 | **v0.5.6 前の docs 整理（外部レビューの検証）**: §3.2 の独り言ストックの 30 日失効の出典表記「（spec §4.4.4）」を「本書の設計値」へ直した（spec §4.4.4 は発話時に鮮度を判定することだけを定め、しきい値は持たない）。設計内容の変更はなし。 |