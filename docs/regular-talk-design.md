...
**状態**: 設計 v1.3 / **M11・M12 実装済み（2026-07-24、ブランチ `feat/v0.3-regular-talk`）**。cargo test 248 / tsc green・ライブ API 検証済み・architecture.md v1.6 追随済み。**v0.3.0 として出荷**（2026-07-24、タグ `v0.3.0`。実機 UI 目視はリリース監査で PASS）。
**作成日**: 2026-07-23
...
- **既存コードの実態を正とする**（daily-support-design §0 と同じ運用）。本書は**設計時点の**ソース（v0.2.0、schema v8、SpeechCategory 9 カテゴリ）を基準に設計し、architecture.md（v1.4）は実装時に v1.5 として追随改訂した（実施済み）。
...
- 夜間静音帯内に設定時刻を置いた枠は、tick ごとに gate 段 2 で `Deferred` になり続け、その日は配達されない（spec 織り込み済みの挙動。UI 警告 §9.3 が対応）。
- **静音プリフィルタ**（リリース監査で追加・v1.1）: 発火条件成立後・材料集約の前に `governance::ambient_quiet_now()`（段 1 ハード静音 + 段 2 夜間静音の安価な合成判定。**ゲートではない** — 会計を持たず、正式判定は従来どおり deliver_event 内の 1 回）で早期 return する。これが無いと、抑制中（フルスクリーン利用・夜間静音帯内設定）に条件成立が続く間、**材料集約（天気取得）+ advanced の LLM 整形が毎 tick 反復**し（最長 6h の失効窓 × 毎分）、§13.1-7 のコスト受容前提（1 日最大 2 回）が崩れる。プリフィルタ後の空振り tick のコストは判定 1 回で無視できる。降雨の一言（§5.6）にも同じプリフィルタを適用する。
...
7. **advanced 整形のコスト上限の扱い → M12 で裁定（受容）、v0.3 リリース監査で前提を修復**: `polish_script` は LLM コストを `api_usage` に記録するが、`monthly_limit_usd` の閾値チェック（80% 警告・自動降格）はチャット経路（`degraded_until` を立てる）でしか発火しない。この受容は「1 日最大 2 回の短い言い換え ≈ 月 $0.004」という発火頻度を前提にした裁定だが、リリース監査で **gate 手前の LLM 呼び出しが Deferred 反復時に毎 tick 走り（最大 ~360 回/日）前提を崩す**ことが判明 → **静音プリフィルタ（§5.2・`ambient_quiet_now`）で前提を実装側が保証する形に修正**（Fable 裁定: 受容裁定の改訂ではなく実装修正を選択）。頻度が設計どおりに戻ったため、閾値チェック非経由の受容は維持。厳密なコスト保護が必要になれば「月次上限超過ならスキップして low へ」の軽量チェックを足す（将来判断）。

   **★v0.5 で決着（この受容は失効）**: 月額上限は LLM 呼び出し前の単一ゲートになり、**この経路も通るようになった**（`regular_talk::polish_script` が `dialogue::cost_exceeded` を呼び、超過時は黙って言い換えを諦める）。ここに書かれた「閾値チェックはチャット経路でしか発火しない」「将来判断」は**もう実態と違う**。spec §4.2.7 / `docs/release-notes/v0.5.0.md` の項目 2 を参照。
...
| 2026-07-24 | v1.1 | v0.3 リリース監査（release-audit）の Medium 指摘を Fable 裁定で反映: **静音プリフィルタ** `governance::ambient_quiet_now` を新設し、定例会話（朝/夜）・降雨の一言の材料集約・LLM 整形の前で早期 return（§5.2）。抑制中の毎 tick LLM 反復を止め、§13.1-7 のコスト受容前提（1 日最大 2 回）を実装側で保証する。ゲートの会計は従来どおり deliver_event 内 1 回（二重ゲート禁止は不変） |
| 2026-09-05 | v1.2 | **docs 整理（tidy-docs、v0.5.2 タグ後）**: §13.1 項目 7 の「advanced 整形のコスト上限は閾値チェック非経由（受容）」が **v0.5 で失効している**ことを追記。月額上限は LLM 呼び出し前の単一ゲートになり、`polish_script` も`dialogue::cost_exceeded` を通る。設計判断の記録は残し、失効した旨を追記する形にした（履歴を書き換えない）。設計本文の変更はなし。 |
| 2026-09-14 | v1.3 | **docs 整理（tidy-docs、v0.5.5 タグ後）**: 冒頭の状態行が出荷前の「残: 実機 UI 目視確認・v0.3 リリース」のままだったので、v0.3.0 で出荷済みと明記。設計内容の変更はなし。 |
