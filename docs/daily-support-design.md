...
**状態**: 設計 v2（未実装）。v1 に対する 2 回の反証（社内 reviewer + 外部レビュー）を反映。実装着手時に本書の契約を architecture.md へ反映する。
...
| `add_todo` / `list_todos` / `complete_todo` / `delete_todo` / `update_todo` | 各種 | `TodoEntry[]` | 4.6.2 |
...
| **M9** | 状況発話 + 検知（§4.6.3） | `presence/context.rs`（OS アイドル/連続利用/バッテリー）、`spawn_context_watcher`、状況カテゴリ、ガバナンス設定 UI、辞書 `situation_*`、フィードバック導線 |
...
1. リマインダー自然文パーサの**語彙・書式の網羅表**（時間帯マッピング既定値、全角処理、あいまい表現の切り分け）。
2. 状況検知の**閾値**（連続利用 90 分？ バッテリー 15%？ アイドル境界 5 分？）。
3. Situation* の**連投ウィンドウ・回数**（M9）。
4. 音量/ミュート検知（`system_muted`）の**要否**（優先度低、無くても成立）。
5. ToDo 件数バッジの採否（既定 OFF、次段階送り可）。
6. 起動時回収の**集約 or 個別**（過ぎた期限が複数のときの通知まとめ方。§7.1 は「直近 1 件に集約」を既定案とする）。
7. reminder_log の**保持上限**（既定 500 案）と once 完了 reminders の**物理削除タイミング**。
