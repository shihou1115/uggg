...
**状態**: 設計 v2.4。**M7〜M10 すべて実装済み**（M7 統合リマインダー / M8 ToDo・日課 / M9 状況発話+ガバナンス / M10 カレンダー参照。2026-07-18、architecture.md v1.4 に契約反映済み。実装時の確定判断は §11.1 と architecture §2/§11.4 の注記参照）。**Tier S 4 機能そろい、v0.2.0 として出荷**（2026-07-18、タグ `v0.2.0`）。
...
### 3.2 発話失敗時のフォールバック（spec §3.1 のフォールバック）
...
| v2.4 | 2026-09-24 | §3.2 の見出しの「spec §3.1 の二段構え」を「spec §3.1 のフォールバック」へ。spec §3.1 の「重大なものはトーストで二段表示」が v0.5.6 で取り下げられたため、紛らわしくないよう呼び方を改めた（ここが指すのは発話できないときにトーストへ落とすフォールバックで、取り下げの対象ではない）。設計内容の変更はなし。 |
