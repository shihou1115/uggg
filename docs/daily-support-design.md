...
**状態**: 設計 v2。**M7（共通基盤 + 統合リマインダー）・M8（ToDo・日課管理）・M9（状況発話 + 検知 + ガバナンス完成）実装済み**（2026-07-17、architecture.md v1.3 に契約反映済み。実装時の確定判断は §11.1 と architecture §2/§11.4 の注記参照）。M10（カレンダー）は未実装。
