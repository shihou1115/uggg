...
| `menu/context-menu.ts` | メニュー項目の生成・各アクションの dispatch |
...
- [ ] **A-6 ウインドウ位置**: 移動後に終了→再起動で位置復元、モニタ構成変更で中央に戻る
...
- [ ] **C-4 ドラッグ**: 透過部分ではドラッグできない、キャラ本体でドラッグ→ウインドウ移動
- [ ] **C-5 コンテキストメニュー**: 右クリックで複数項目（話す/設定/静音/隠す/ゴースト切替/終了）
...
- [ ] **E-3 ツール**: tools_enabled で「今何時?」「N分後に教えて」「📋ボタン+翻訳」が動く
