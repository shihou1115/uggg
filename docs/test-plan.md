...
- [ ] **E-5b データの破損検知（spec §4.5.5／v0.5.1）**: **正常な DB では 設定 →「システム」→「データ管理」に何も出ず、`%APPDATA%\ugg\` に退避コピーが作られないこと**（毎起動コピーを撒かない）。破損 DB での確認は**必ず隔離した APPDATA の複製で行う**（実データを壊さない）: 検知内容と退避先が表示され、① 原本のタイムスタンプ付きコピー ② 救出コピー が作られ、**元の DB は作り直されず、リマインダー・ToDo・記憶が残ったままアプリが起動する**こと。**さらに 2 回目以降の起動で退避が増えないこと**（破損した DB を抱えたまま使い続けると、毎起動コピーでディスクを食い潰す。既存の退避があれば作り直さない）。退避コピーに `-wal` が付いてくることも確認する
