...
### Step 9: リリース後の docs 整理 (タグ打ち・push 後)

タグを打って push したら、`.claude/skills/tidy-docs` に従って docs/ を整理する
(完遂した実装計画・消費済みインプットの削除、正本の外に生えた記録の正本への統合)。
**リリースの完了条件には含めない** (hygiene で出荷を遅らせない) が、次バージョンの
スコープ選定前に済ませる。タグ後に行うのは「原本はタグ vX.Y.Z 以前の履歴にある」と
復元先を一言で言えるようにするため。
