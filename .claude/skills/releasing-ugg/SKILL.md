...
この skill にモデル名は書かない。モデルは `.claude/agents/` の各エージェント定義側で固定されている
(全体方針は docs/ai_model_routing.md)。

- **最初に Workflow `release-audit` を実行する**（`.claude/workflows/release-audit.js`）。
  Step 1〜4 相当の機械検査（ビルド・契約突合・バージョン整合・bundle 設定）と直近タグ以降の
  差分レビューを並列で済ませ、判定材料をそろえる。
- Step 2 / 5 / 7 の機械的作業（バージョン書き換え・ビルド実行・ハッシュ記録）は
  `implementer` / `build-checker` / `mechanic` エージェントに委譲してよい。
- **Step 6 の起動確認結果の解釈と、完了判定（出荷可否）はメインセッションが行う。**
  出荷判定の際は CLAUDE.md の切替ルールに従い、メインを Fable 5 へ切り替える提案をする。
...
### Step 9: docs 整理 (タグ打ち後・**push 前**)

**順序は「タグ → tidy-docs → push」**(2026-09-11 ユーザー指示で push 後から変更)。
`.claude/skills/tidy-docs` に従って docs/ を整理する (完遂した実装計画・消費済み
インプットの削除、正本の外に生えた記録の正本への統合)。

- **タグの後に行う**のは「原本はタグ vX.Y.Z 以前の履歴にある」と復元先を一言で
  言えるようにするため。タグが整理前の状態を保存する。
- **push の前に行う**のは、整理のコミットを同じ push で出すため。後回しにすると
  push が 2 回に分かれ、片方を忘れる。
- **リリースの完了条件には含めない** (hygiene で出荷を遅らせない)。整理で判断に
  迷うものが出たら、タグは打ったまま push だけ先に済ませてよい。
...
- [ ] **push の前に `tidy-docs` を通した** (Step 9。完了条件ではないが、push を
      2 回に分けないためここで済ませる)
