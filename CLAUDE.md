...
| **4** | **実装着手**（垂直スライス・M0〜M6） | src/, src-tauri/src/ | ✅ M0〜M6 完了、**v0.1.4 リリース済**（2026-07-11、タグ `v0.1.4`）。**M7〜M9（リマインダー / ToDo・日課 / 状況発話+ガバナンス）実装済み**（2026-07-17、M7+M8 はコミット `1ab7e01`、M9 未コミット。次は M10 カレンダー → v0.2 リリース） |
...
| docs/architecture.md | モジュール構成・契約・設計判断 | v1.3 ✅（M7〜M9 契約反映済み） |
| docs/test-plan.md | テスト戦略・手動チェックリスト | v1 ✅ |
| docs/implementation-plan.md | 実装計画（M0〜M6 マイルストーン） | v1 ✅ |
| docs/daily-support-design.md | **日常支援 Tier S の Phase 2 設計書**（§4.6 実装契約・DB・M7〜M10） | 設計 v2 ✅（**M7〜M9 実装済み**、M10 未実装） |
