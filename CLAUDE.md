...
| docs/architecture.md | モジュール構成・契約・設計判断 | v2.11 ✅（M7〜M14 + v0.5 系。`get_cost_status`、月次告知キー 2 つ、`ghost.json` の persona / prompt / `default_shell`、`talk_poses`、`get_db_health`、export schema v2 を契約表に反映。**§7.0 に WebView2 自動再生の前提**（`additionalBrowserArgs` は wry 既定を置換する）、§6.2 に `recall` の実装契約。**v2.4 で破損 DB 時のマイグレーション扱いを追記**） |
