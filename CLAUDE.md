...
| ファイル | 役割 | 状態 |
|---|---|---|
| docs/spec.md | 要件の正本 | v1.5.5 ✅（**§6.0 に v0.5 スコープ**＝契約と実装の一致。**§4.2.2b に persona**、**§4.2.7 に上限の事前ゲート化と月次リセット**、**§4.6.1 に配達の可視性**、**§5 にログ**。§6.4 に伺かゴースト変換の非目標と「他人のキャラの人格記述の自動生成」レッドライン、§6.5 に `crates/nar2ugg/` の凍結） |
| docs/architecture.md | モジュール構成・契約・設計判断 | v2.1 ✅（M7〜M14 + v0.5。`get_cost_status`、`app_settings` の月次告知キー 2 つ、`ghost.json` の persona / prompt を契約表に反映） |
...
| docs/test-plan.md | テスト戦略・手動チェックリスト | v1.9 ✅（§5 に A〜G 全節。モニタ選択 A-12〜14、advanced 独り言 D-4b〜e / 時事ネタ D-6b〜c、**配達の可視性 G-2b / 保留の集約 G-2c（v0.5）**、§5.9 実機検証記録） |
