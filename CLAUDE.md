...
| **v0.5** | 宣言どおりに動く（spec §6.0、2026-09-02 スコープ確定） | spec v1.5 §6.0 | 🚧 **着手**。**新機能ゼロ。契約と実装を一致させる回。** 契約は宣言されているのに実装が繋がっていない箇所が **12 件** 見つかった（既知の keyring / persona / 口パクと同じ型で、「それらしい名前のテスト」や「成功を返す UI」が発覚を遅らせる共通構造）。① **`persona` / `prompt` の配線** — serde が未宣言で黙って捨てており、**advanced の LLM はキャラの人格を名前以上に知らない** ② **月額上限を本当に効かせる** — 実質「5 分の一時停止」でその月ずっと課金される ③ **記憶の閲覧・追加・削除 UI** — コマンドは実装済みでフロント呼び出しが 0 件 ④ **配達の可視性判定** — 「Windows トースト」の実体がアプリ内の帯で、隠している間に単発リマインダーが無言で完了扱いになる ⑤ **再発を止める土台** — ログ + 契約テスト 2 本。**実ユーザーは開発者本人のみ**（2026-09-02 確認）のため配布まわり（コード署名・更新導線・SmartScreen）は扱わない |
...
| ファイル | 役割 | 状態 |
|---|---|---|
| docs/spec.md | 要件の正本 | v1.5 ✅（**§6.0 に v0.5 スコープ**＝契約と実装の一致。**§6.4 に伺かゴースト変換の非目標**と「他人のキャラの人格記述の自動生成」レッドライン、**§6.5 に変換ツール `crates/nar2ugg/` の凍結**） |
