...
/// こちらは **変わったものだけを入れ替える**。所要は何が変わったか次第で、パッケージだけなら
/// 短いが、**AI モデルが変わると数 GB になる**（v0.5.5 からモデルも対象）。
async function onIrodoriUpdate(): Promise<void> {
  if (!inputs) return;
  // **戻せる範囲を正直に書く**（2026-09-14 監査で発覚）。退避して戻せるのは GitHub から取る
  // ランタイム本体の 3 つだけで、個別のパッケージが連鎖して入れ替えた依存までは戻せない。
  // 以前は一律に「いまの状態へ戻します」と書いており、同意の本文が実装より先行していた。
  const ok = await uggConfirm(
    "Irodori-TTS のランタイムに更新があります。変わったものだけを入れ直します。\n" +
      "AI モデルが変わっているときは数 GB の取得になります。更新中は VOICEVOX の声で話します。\n" +
      "ランタイム本体は、入れ直しに失敗したら元に戻します（個別のパッケージは元の版へ戻せないことがあります）。続行しますか?",
    "更新確認",
  );
  if (!ok) return;
